...
public class TestQuery : MonoBehaviour
{
    private void Start()
    {
        ...
        */
        string answer = MyDataBase.ExecuteQueryWithAnswer($"SELECT Region_Geo FROM Geo WHERE Id_Geo = 21;");
        string answer1 = MyDataBase.ExecuteQueryWithAnswer($"SELECT January FROM Insolation WHERE Id_Insolation = 1;");
